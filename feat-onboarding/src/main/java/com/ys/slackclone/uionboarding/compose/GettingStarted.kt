...
import com.ys.slackclone.commonui.theme.SlackLogoYellow
...
@Composable
private fun ImageExitTransition() = shrinkOut() + fadeOut()
...
private fun GetStartedButton(composeNavigator: ComposeNavigator) {
	...
	AnimatedVisibility(
		visible = expanded,
		enter = GetStartedEnterTransition(density),
		exit = GetStartedExitTransition()
	) {
		Button(
			onClick = {
...
@Composable
private fun GetStartedExitTransition() = slideOutVertically() + shrinkVertically() + fadeOut()
...
private fun IntroText(modifier: Modifier = Modifier) {
	...
	AnimatedVisibility(
		visible = expanded,
		enter = IntroEnterTransition(density),
